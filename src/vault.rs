...
use chacha20poly1305::{XChaCha20Poly1305, XNonce, KeyInit as XKeyInit, aead::Aead as XAead};
use rand::RngCore;
...
pub fn wrap_vault_key_with_master_key(vault_key: &VaultKey, master_key: &MasterKey) -> Result<Vec<u8>, CryptoError> {
    let cipher = XChaCha20Poly1305::new_from_slice(&master_key.0)
        .map_err(|_| CryptoError::KeyWrap)?;
    let mut nonce_bytes = [0u8; XCHACHA_NONCE_LEN];
    rand::rngs::OsRng.fill_bytes(&mut nonce_bytes);
    let nonce = XNonce::from_slice(&nonce_bytes);
    let mut ciphertext = cipher.encrypt(nonce, vault_key.0.as_ref())
        .map_err(|_| CryptoError::KeyWrap)?;
...
pub fn unwrap_vault_key_with_master_key(wrapped: &[u8], master_key: &MasterKey) -> Result<VaultKey, CryptoError> {
    ...
    let plaintext = cipher.decrypt(nonce, &wrapped[XCHACHA_NONCE_LEN..])
        .map_err(|_| CryptoError::KeyUnwrap)?;
    if plaintext.len() != VAULT_KEY_LEN {
        return Err(CryptoError::InvalidInput("decrypted key wrong length".into()));
    }
    let mut key = [0u8; VAULT_KEY_LEN];
